...
`tests/fixtures/reference_mlp.onnx` is a real opset-18 model generated by
`tools/generate_reference_fixture.py`. The generator validates the model with
ONNX, executes it with ONNX Runtime 1.22.1, and writes raw Float32 input/output
fixtures. The C++ smoke test imports the serialized model and requires every
native output to match the trusted output within `1e-6`. The Python packages are
fixture-generation tools only and are not production dependencies.
